...
static void fs_sync() {
  ...
  write_sectors_ATA_PIO((uint32_t)fs_dir, FS_DIR_ENTRY_LBA,
                        sizeof(fs_dir) / 512);
}
...
void fs_init() {
  ...
  if (fs_super.magic == FS_MAGIC) {
    ...
    read_sectors_ATA_PIO((uint32_t)fs_dir, FS_DIR_ENTRY_LBA,
                         sizeof(fs_dir) / 512);
  } else {
...
int fs_write_file(const char *filename, const uint8_t *data, uint32_t size) {
  ...
  for (int i = 0; i < MAX_FILES; i++) {
    if (fs_dir[i].flags == 1 && string_compare(fs_dir[i].name, filename)) {
      slot = i;
      break; // Overwrite existing (WARNING: leaks old data blocks in this
             // simple FS)
    }
...
void fs_list_files() {
  ...
  uint32_t used_kb =
      ((fs_super.next_free_lba - FS_DATA_START_LBA) * 512) / 1024;
  print_number(used_kb, 0x0E);
  print_string(" KB\n", 0x0F);
}

// ============================================================================
// Phase 4: Vector-based file search (BoW Hashing Trick)
// ============================================================================
#define VECTOR_DIM 256

static void compute_document_vector(const char *text, int len, float *vec) {
...
void fs_vector_search(const char *query) {
  if (!fs_initialized) {
    print_string("[Vector Search] NeuralFS is not mounted.\n", 0x0C);
    return;
  }

  print_string("[Vector Search] Query: '", 0x0B);
  print_string(query, 0x0F);
  print_string("'\n", 0x0B);
  ...
  float qvec[VECTOR_DIM];
  compute_document_vector(query, qlen, qvec);
  ...
  for (int i = 0; i < MAX_FILES; i++) {
    if (fs_dir[i].flags == 1) {
      uint8_t *file_data;
      int size = fs_read_file(fs_dir[i].name, &file_data);
      if (size > 0) {
        float fvec[VECTOR_DIM];
        compute_document_vector((const char *)file_data, size, fvec);

        float score = 0.0f;
        // Cosine similarity (since arrays are L2 normalized, it's just dot
        // product)
  ...
  if (best_idx != -1 && best_score > 0.01f) {
    print_string(" -> Best Match: ", 0x0A);
    print_string(fs_dir[best_idx].name, 0x0E);
    print_string(" (Similarity: ", 0x07);
    print_number((int)(best_score * 100), 0x07);
    print_string("%)\n", 0x07);
  } else {
    print_string(" -> No relevant matches found.\n", 0x0C);
  }
}
