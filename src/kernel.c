...
void kernel_main(uint32_t magic, uint32_t ebx_mboot_ptr) {
  ...
  int has_llama = 0;
  {
    ...
    if (q8cfg[0] > 0 && q8cfg[0] < 10000 && q8cfg[2] > 0 && q8cfg[2] < 100 &&
        marker[0] == 'N' && marker[1] == 'N' && marker[2] == 'Q' &&
        marker[3] == '8') {
  ...
  if (has_llama) {
    print_string("\n[System] NeuralOS Agentic Edition — AI Agent Active\n",
                 0x0A);
    print_string(
        "[System] Talk naturally! I understand intent, not just commands.\n",
        0x0E);
  } else {
  ...
  while (1) {
    ...
    if (ci_compare(prompt_buffer, "LLAMA")) {
      if (has_llama) {
        ...
        llama_generate(256, 0.8f);
      } else {
        print_string("[!] Llama2 not loaded.\n", 0x0C);
      }
    } else if (string_starts_with(prompt_buffer, "LLAMA ") ||
               string_starts_with(prompt_buffer, "llama ")) {
      if (has_llama) {
        print_string("\n[Llama2 Agent] ", 0x05);
        llama_generate_with_prompt(prompt_buffer + 6, 256, 0.8f);
      } else {
        print_string("[!] Llama2 not loaded.\n", 0x0C);
      }
    } else {
